<template>
    <div>
        ...
        <hr>
        <div class="cards-wrapper">
            <TransactionRecordView v-for="trans in transHiss1" :key="trans.transactionId ?? undefined" :trans="trans"
                @read="handleRead" @unread="handleUnread" @checkTrade="handlecheck" />
        </div>
    </div>
</template>
...
async function handlecheck(trans: TransactionDto): Promise<void> {
    ...
    try {
        if (trans.transactionId !== null) {
            ...
            result = await cheakTrans(trans.transactionId);
    ...
    if (result == null||(result!=null&&result.data.transactionId==0)) {
            alert("Attention , Transaction not found");

    }else{

            if (result.data.transactionId != trans.transactionId) {
                alert(`数据被篡改
                交易ID: ${result.data.transactionId}
                `);
            }else            if (result.data.collectibleId != trans.collectibleId) {
                alert(`数据被篡改
                物品ID: ${result.data.collectibleId}
                `);
            }else            if (result.data.buyerId != trans.buyerId) {
                alert(`数据被篡改
                买家ID: ${result.data.buyerId}
                 `);
            } else            if (result.data.sellerId != trans.sellerId) {
                alert(`数据被篡改
                卖家ID: ${result.data.sellerId}
                `);
            } else            if (result.data.transactionDate != trans.transactionDate) {
                alert(`数据被篡改
             交易时间: ${result.data.collectibleId}
                `);
            }else{
            alert(`区块链交易信息：
                    交易ID: ${result.data.transactionId}
